...
# Automate artillery/k8s load test based on
...
usage()
{
    echo "Usage: deploy.sh [PARAMETERS] [TEST_FILE]"
    echo
    echo "[TEST_FILE]                 Test yaml file, optional."
    echo
    echo "PARAMETERS"
    echo "  -p | --project             Google Cloud project name, required"
    echo "  -c | --cluster             Google Cloud cluster name, default is loadtest-artillery"
...
GCE_CLUSTER=loadtest-artillery
GCE_CLUSTER_SIZE=3
...
if [ -f "$TEST_FILE" ]; then
  [[ "$TEST_FILE" != *.yaml ]] && ERROR="paths file "\""$TEST_FILE"\"" is not a yaml file."
  [ -n "$ERROR" ] && echo "Error: $ERROR" && exit 1
fi
...
if [ -z "$GCE_TAG" ]; then
  PATHS_NAME=
  if [ -f "$TEST_FILE" ]; then
    PATHS_NAME=$(basename "$TEST_FILE" | sed "s/.yaml$//")
  fi
...
echo "======================================================"
echo "Will prepare artillery testing machines on:"
...
if [ -n "$DOES_CLUSTER_EXISTS" ]; then
  ...
  gcloud container clusters create "$GCE_CLUSTER" --project "$GCE_PROJECT" --zone "$GCE_ZONE" --machine-type "$GCE_MACHINE" --disk-size "20" --num-nodes "$GCE_CLUSTER_SIZE" --enable-cloud-logging --enable-cloud-monitoring --no-enable-ip-alias
fi
...
if [ -f "$TEST_FILE" ]; then
  cp "$TEST_FILE" "$SCRIPTPATH/docker-image/"
fi
docker build -t "gcr.io/$GCE_PROJECT/artillery-workers" "$SCRIPTPATH/docker-image"

# upload docker
echo "======================================================"
echo ">>> pushing docker image..."
gcloud docker --project "$GCE_PROJECT" -- push "gcr.io/$GCE_PROJECT/artillery-workers"

gcloud beta container images add-tag "gcr.io/$GCE_PROJECT/artillery-workers" "gcr.io/$GCE_PROJECT/artillery-workers:$REGISTRY_TAG" --project "$GCE_PROJECT" --quiet

# update k8s config files
echo "======================================================"
echo ">>> preparing kubernetes config files..."
cat "$SCRIPTPATH/kubernetes-config/artillery-worker-controller.tpl.yaml" | sed "s/GCE_PROJECT/${GCE_PROJECT}/g" | sed "s/GCE_CLUSTER_SIZE/${GCE_CLUSTER_SIZE}/g" > "$SCRIPTPATH/kubernetes-config/artillery-worker-controller.yaml"

echo "======================================================"
echo ">>> deploying worker..."
K8S_EXISTS=$(kubectl describe rc/artillery-worker)
if [ -n "$K8S_EXISTS" ]; then
  echo "    deleting existing worker..."
  kubectl delete rc/artillery-worker
fi
echo "    creating worker..."
kubectl create -f "$SCRIPTPATH/kubernetes-config/artillery-worker-controller.yaml"
